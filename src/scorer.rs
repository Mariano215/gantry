...
fn pred_matches(
    ...
    for ev in events {
        ...
        if let Some(pointer) = &pred.pointer {
            let at = ev["_subject"].pointer(pointer).cloned().unwrap_or(Value::Null);
...
impl ScoreSnapshot {
    ...
    pub fn markdown(&self) -> String {
        ...
        for p in &self.scores {
            let score = p.score.map(|n| n.to_string()).unwrap_or_else(|| "N/A".into());
...
mod tests {
    ...
    fn a_seam_lifts_observability_to_4() {
        let events = vec![
            ev("e0", "r1", "model.call", json!({})),
            ev("e1", "r2", "run.open", json!({ "restored_checkpoint": "r1-ckpt-1" })),
        ];
        let snap = rules().score(&events);
        let p11 = snap.scores.iter().find(|p| p.primitive == 11).unwrap();
    ...
    fn overall_is_the_minimum_not_the_average() {
        ...
        let events = vec![
            ev("e0", "r1", "model.call", json!({})),
            ev("e1", "r2", "run.open", json!({ "restored_checkpoint": "c" })),
