...
- **Nothing enforces that the console's claims match the API.** The six views
  were verified by looking at them once. A shape change in `src/console.rs`
  would show up as a blank cell, not a test failure. The check that should
  exist is a headless render against a fixture ledger asserting known values
  appear on screen, run in CI.
